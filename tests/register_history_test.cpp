...
int main()
{
    for (int address_bytes : {0, 1})
    {
        for (bool auto_increment : {false, true})
        {
            ...
            device.DataByte(0x12, 1);
            device.DataByte(0x34, 2);
            device.CallThisEachI2CStopCondition();
            device.CallThisEachI2CAddrByteWrite(true);
            device.DataByte(0x56, 3);
            device.DataByte(0x78, 3); // Equal timestamps must keep capture order.
            device.CallThisEachI2CStopCondition();
            ...
            check(first.timestamp == 1 && second.timestamp == 2 && third.timestamp == 3 && fourth.timestamp == 3);
            ...
            check(device.GetSnapshotViewAt(0.5).registers.empty());
            check(device.GetSnapshotViewAt(1).registers.at(0).data_w == first.register_info.data_w);
            check(!device.GetSnapshotViewAt(3).is_write);
        }
    }
    ...
    repeated_start_device.DataByte(0xAA, 1);
    repeated_start_device.CallThisEachI2CAddrByteWrite(true);
    repeated_start_device.DataByte(0xBB, 2);
